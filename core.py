...
def use_model(model_name, config_file_path, model_file_path, vocab_file_path, num_labels):
    ...
    if(model_name == 'bert'):
        ...
        model = model_class.from_pretrained(model_file_path, force_download=True)
...
def convert_data_to_feature(tokenizer, train_data_path):
    ...
    with FileWrapper(open(train_data_path,'rb'))as f:
    ...
    output = open('work/taipei-QA-BERT/trained_model/data_features.pkl', 'wb')
